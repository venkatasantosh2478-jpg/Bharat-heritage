...
export default function Translator() {
  ...
  const [useOfflineMode, setUseOfflineMode] = useState(false); // Cloud AI by default with offline edge fallback
  const [selectedAiModel, setSelectedAiModel] = useState("gemini-3.8-flash");
  const [engineUsed, setEngineUsed] = useState("gemini-3.8-flash");
  ...
  const [conversation, setConversation] = useState([
    {
      sender: "system",
      text: "⚡ Dual Engine Multilingual Matrix (Gemini AI + Offline Edge Lexicon) active. Supports Telugu, Telugu in English (Teluglish), Hindi, Tamil, and 10+ regional languages.",
      time: "Now",
    }
  ]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-2xl bg-muted border border-border">
                <Cpu className="w-3.5 h-3.5 text-primary" />
                <span className="text-[11px] font-bold text-muted-foreground uppercase">AI Model:</span>
                <select
                  value={selectedAiModel}
                  onChange={(e) => {
                    setSelectedAiModel(e.target.value);
                    if (useOfflineMode) setUseOfflineMode(false);
                  }}
                  className="bg-transparent text-xs font-semibold text-foreground outline-none cursor-pointer pr-1"
                >
                  <option value="gemini-3.8-flash" className="bg-background text-foreground">Gemini 3.8 Flash (Ultra Fast)</option>
                  <option value="gemini-3.1-pro-preview" className="bg-background text-foreground">Gemini 3.1 Pro (Deep Telugu & Reasoning)</option>
                  <option value="gemini-3.1-flash-lite" className="bg-background text-foreground">Gemini 3.1 Flash Lite</option>
                  <option value="gemini-2.5-flash" className="bg-background text-foreground">Gemini 2.5 Flash</option>
                </select>
              </div>

              <button
                onClick={() => setUseOfflineMode(!useOfflineMode)}
                className={`px-3.5 py-2 rounded-2xl flex items-center gap-2 text-xs font-bold transition-all border cursor-pointer ${
                  useOfflineMode 
